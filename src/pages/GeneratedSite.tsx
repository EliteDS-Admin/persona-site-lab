...
import type { Inspiration, StructuredProfile } from '@/contexts/SiteFactoryContext';
    ...
    <div className="relative min-h-screen overflow-hidden text-white" style={{ background: colors.gradient }}>
      ...
      <header className="relative z-10 border-b border-white/10 bg-black/20 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
          ...
          <Button
            ...
            onClick={() => window.open('https://wa.me/?text=' + encodeURIComponent(`Bonjour, je viens de découvrir mon site ${window.location.href}`), '_blank')}
          >
            Discuter
          </Button>
        </div>
      ...
      <main className="relative z-10">
        {sanitizedHtml ? (
          ...
          <>
            <section className="mx-auto max-w-6xl px-6 pb-20 pt-24 text-center sm:text-left">
              <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs uppercase tracking-[0.3em] text-white/70">
                  {profile.title}
                </span>
                <h1 className="mt-6 text-4xl font-black tracking-tight text-white sm:text-5xl lg:text-6xl">{profile.name}</h1>
                ...
                <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-start">
                  <Button
                    size="lg"
                    className="gap-3 rounded-full bg-white px-8 py-6 text-sm font-semibold uppercase tracking-[0.3em] text-black hover:bg-white/90"
                    onClick={() => profile.cta.link && window.open(profile.cta.link, '_blank')}
                    disabled={!profile.cta.link}
                  >
                    {profile.cta.text}
                  ...
                  <a
                    href="#inspirations"
                    className="text-xs uppercase tracking-[0.3em] text-white/70 underline-offset-4 hover:text-white"
                  >
            ...
            <section className="mx-auto max-w-6xl px-6 pb-24">
              <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                <motion.div
                  ...
                  className="space-y-6 rounded-3xl border border-white/10 bg-black/30 p-10 shadow-[0_40px_120px_rgba(6,8,7,0.55)]"
                >
                  {profile.sections.map((section) => (
                    <div key={section.title} className="rounded-2xl border border-white/5 bg-white/5 p-6">
                      <h2 className="text-lg font-semibold uppercase tracking-[0.3em] text-white/80">{section.title}</h2>
                      <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-white/70">{section.content}</p>
                    </div>
                  ))}
                </motion.div>
                ...
                >
                  <div className="rounded-3xl border border-white/10 bg-black/40 p-8">
                    <h3 className="text-sm font-semibold uppercase tracking-[0.4em] text-white/60">Ambiance</h3>
                    <div className="mt-6 flex flex-wrap gap-4">
                      <div className="flex flex-col items-center gap-2">
                        <span className="text-xs uppercase tracking-[0.3em] text-white/40">Primaire</span>
                        <span className="h-12 w-12 rounded-full" style={{ backgroundColor: colors.primary }} />
                      </div>
                      <div className="flex flex-col items-center gap-2">
                        <span className="text-xs uppercase tracking-[0.3em] text-white/40">Secondaire</span>
                        <span className="h-12 w-12 rounded-full" style={{ backgroundColor: colors.secondary }} />
                      </div>
                    </div>
                  </div>

                  <div id="inspirations" className="rounded-3xl border border-white/10 bg-black/40 p-8">
                    <h3 className="text-sm font-semibold uppercase tracking-[0.4em] text-white/60">Inspirations retenues</h3>
                    {inspirations.length === 0 ? (
                      <p className="mt-6 text-sm text-white/60">Vous n'avez pas encore sélectionné d'inspiration.</p>
                    ) : (
                      <div className="mt-6 space-y-4">
                        {inspirations.map((inspiration) => (
                          ...
                          >
                            <p className="text-xs uppercase tracking-[0.4em] text-white/40">{inspiration.domain}</p>
                            <p className="mt-2 text-lg font-semibold text-white">{inspiration.title}</p>
                            <p className="mt-2 text-sm text-white/70">{inspiration.justification}</p>
                            <span className="mt-3 inline-flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/60 group-hover:text-white">
                              Explorer
